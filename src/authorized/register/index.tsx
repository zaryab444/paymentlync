import { Form, Button, Col, Input, Popover, Progress, Row, Select, message , Card , Text , Divider } from 'antd';
...
import Api from "../login/loginService";
...
const Register : any =() => {
  let api = new Api();
  let notification = new NotificationBar();
  ...
  function handleSubmit(event : any) {
    ...
    .then((response : any) => {
      if(!response.data.statuscode){
        notification.openNotification("Success",response.data.statusmessage)
       //localStorage.setItem("UserInfo",JSON.stringify(response.data.response[0]))
       history.push("/authorized/login")
      }
      else{
        notification.openNotification("Failed",response.data.statusmessage)
       // console.log("failed")
      }
    ...
    <Col span={8} lg={8} md={16} xs={20} sm={20}>
        <Card hoverable>
            ...
            <Row style={{marginTop:'11%',marginRight:'3%',marginLeft:'3%'}}>
                ...
                <Form.Item>
                        <Button htmlType={'submit'} href={'/apps/authorized/login'} block>Back</Button>
                  </Form.Item>
                    <Form.Item>
